...
//     // if (!origin) return callback(null, true) // Cho request không có origin (Postman, Mobile)
...
//         `${origin} không được phép theo chính sách CORS của chúng tôi.`
//       )
//     )
//   },
...
export const corsOptions = {
  origin: function (origin, callback) {
    ...
    return callback(
      new ApiError(
        StatusCodes.FORBIDDEN,
        `${origin} không được phép theo chính sách CORS`
      )
    )
  },
  optionsSuccessStatus: 200,
